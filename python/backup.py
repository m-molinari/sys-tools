...
parser.add_argument("-s", "--sourcedir", type=str, help="What you want backup", default="/etc", required=True)
...
try:
    with tarfile.open(DESTDIR + HOSTNAME + '-etc-' + BACKUPTIME + '.tar.gz', mode='w:gz') as archive:
        archive.add('/etc/', recursive=True)
except:
...
try:
    for files_check in list_of_files:
        ...
        if files_check.startswith('serverino-etc-') and files_check.endswith('gz'):
...
if count > 2:
    for backup_files in list_of_files:
        ...
        if(file_time < current_time - day*RETENTION):
            if backup_files.startswith('serverino-etc-') and backup_files.endswith('gz'):
